...
const PLANS = [
  {
    tier: 'Starter', name: 'Landing Page', monthly: '2,5jt', annual: '2jt',
    period: 'one-time payment · delivered in 3–5 days', featured: false,
    waMsg: 'Hi!%20I%27m%20interested%20in%20the%20Landing%20Page%20package%20(Rp%202.5jt).',
  ...
  {
    tier: 'Professional', name: 'Company Website', monthly: '7,5jt', annual: '6jt',
    period: 'one-time payment · delivered in 7–10 days', featured: true,
    waMsg: 'Hi!%20I%27m%20interested%20in%20the%20Company%20Website%20package%20(Rp%207.5jt).',
  ...
  {
    tier: 'Premium', name: 'Full Platform', monthly: 'Custom', annual: 'Custom',
    period: 'based on project scope', featured: false,
    waMsg: "Hi!%20I'm%20interested%20in%20a%20custom%20web%20platform.%20Let's%20talk!",
...
const FAQS = [
  { q: 'How do I get started?', a: "Just drop us a message on WhatsApp or fill in the contact form. We'll schedule a free 30-minute discovery call to understand your project, then send you a detailed proposal and quote." },
  { q: 'How do I pay? What payment methods are accepted?', a: 'We accept bank transfer (BCA, Mandiri, BRI), GoPay, OVO, and QRIS. We require a 50% deposit upfront and the remaining 50% upon final approval before launch.' },
  { q: 'Can I see a mockup before paying?', a: "Yes! We provide a design concept / mockup after the 50% deposit. If you're not satisfied with the initial concept, we'll revise it at no extra cost." },
  { q: 'What if I already have a domain and hosting?', a: 'No problem. We can deploy to your existing domain and hosting. If you don\'t have one yet, we can guide you on the best options.' },
  { q: 'Will I be able to update the website myself after launch?', a: 'For Professional and Premium packages, yes — we include a CMS that lets you update text, images, and blog posts without touching any code.' },
  { q: 'Do you work with clients outside Jakarta?', a: 'Absolutely. We work with clients all across Indonesia and internationally. All communication is done via WhatsApp, Zoom, or email.' },
  { q: 'What if I need changes after the site is live?', a: 'Every package includes 14–30 days of post-launch support. After that, you can subscribe to our Monthly Maintenance add-on.' },
]
...
export default function Pricing() {
  ...
  return (
    <>
      <style>{`
        .pricing-header { padding-top:calc(var(--nav-h) + 4rem); padding-bottom:3rem; text-align:center; }
        .billing-toggle { display:inline-flex; align-items:center; gap:1rem; background:var(--surface); border:1px solid var(--border2); border-radius:100px; padding:.4rem .6rem .4rem 1.2rem; margin-bottom:3.5rem; font-size:.85rem; color:var(--text2); }
        .toggle-switch { position:relative; width:44px; height:24px; cursor:pointer; }
        .toggle-track { position:absolute; inset:0; background:var(--surface2); border:1px solid var(--border2); border-radius:100px; transition:background .25s; }
      ...
      <div className="page-section pricing-header">
        ...
        <h1 className="section-title reveal" style={{ fontSize: 'clamp(2.5rem,6vw,5rem)' }}>Simple, honest pricing.</h1>
        <p className="section-sub reveal" style={{ marginLeft: 'auto', marginRight: 'auto', textAlign: 'center' }}>All packages include mobile optimization, fast hosting setup, and 30-day post-launch support. No hidden fees. No surprises.</p>
        <div className="billing-toggle reveal reveal-d1">
          <span>Monthly Retainer</span>
          <div className="toggle-switch" onClick={() => setAnnual(a => !a)} role="switch" aria-checked={annual}>
            <div className={`toggle-track${annual ? ' on' : ''}`} />
            <div className={`toggle-thumb${annual ? ' on' : ''}`} />
          </div>
          <span>Annual <span className="save-badge" style={{ opacity: annual ? 1 : 0.4 }}>Save 20%</span></span>
        </div>
      </div>
      ...
      <div className="page-section" style={{ paddingTop: 0 }}>
        <div className="pricing-grid">
          {PLANS.map((plan, i) => (
            <div key={plan.tier} className={`price-card reveal reveal-d${i + 1}${plan.featured ? ' featured' : ''}`}>
              {plan.featured && <div className="featured-badge">Most Popular</div>}
              <div className="price-tier">{plan.tier}</div>
              <div className="price-name">{plan.name}</div>
              <div className="price-amount">
                {plan.monthly !== 'Custom' && <span className="price-currency">Rp</span>}
                <span className="price-number">{annual ? plan.annual : plan.monthly}</span>
              </div>
              ...
              <a href={`https://wa.me/${WA_NUMBER}?text=${plan.waMsg}`} target="_blank" rel="noopener noreferrer" className="price-btn">
                {plan.tier === 'Premium' ? 'Talk to Us' : 'Get Started'}
              </a>
            </div>
          ))}
        </div>
        ...
        <div className="guarantee-banner reveal">
          ...
          <div className="guarantee-text">
            ...
            <p>If you're not happy with the first mockup, we'll revise it completely — free of charge. We don't ask for full payment until you approve the design. Your money is safe.</p>
          </div>
        </div>
        ...
        <div style={{ textAlign: 'center', marginTop: '4rem', paddingTop: '4rem', borderTop: '1px solid var(--border2)' }} className="reveal">
          <p style={{ color: 'var(--text2)', marginBottom: '.5rem' }}>Still unsure which package is right for you?</p>
          <p style={{ color: 'var(--text3)', fontSize: '.875rem', marginBottom: '1.8rem' }}>We're happy to give you a personalised recommendation — for free, with no obligation.</p>
          <a href={`https://wa.me/${WA_NUMBER}?text=Hi!%20I%27m%20not%20sure%20which%20package%20is%20right%20for%20me.%20Can%20you%20help?`} target="_blank" rel="noopener noreferrer" className="wa-btn">
