...
export default function Contact() {
  ...
  return (
    <>
      <style>{`
        .contact-header { padding-top:calc(var(--nav-h) + 4rem); padding-bottom:3rem; }
        ...
        .contact-card { background:var(--surface); border:1px solid var(--border2); border-radius:22px; padding:2.5rem; position:sticky; top:calc(var(--nav-h) + 2rem); }
      ...
      <div className="page-section contact-header">
        ...
        <h1 className="section-title reveal" style={{ fontSize: 'clamp(2.5rem,6vw,5rem)', maxWidth: '600px' }}>
          Let's build something <span style={{ color: 'var(--purple-light)' }}>great together.</span>
        </h1>
      </div>
      ...
      <div className="page-section" style={{ paddingTop: 0 }}>
        <div className="contact-layout">
          ...
          <div className="contact-info reveal">
            <p>Whether you have a clear brief or just a rough idea — we'd love to hear from you. Most enquiries get a reply within 60 minutes during business hours.</p>
            ...
            <div className="mini-testi-section">
              <h3 className="mini-testi-title">What our clients say</h3>
              <div className="mini-testi-grid">
                {TESTIMONIALS.map(t => (
                  <div key={t.name} className="mini-testi-card">
                    <div className="mini-stars">{t.stars}</div>
                    <p className="mini-quote">{t.quote}</p>
                    <div className="mini-author">
                      <div className="mini-avatar">{t.initials}</div>
                      <div><div className="mini-name">{t.name}</div><div className="mini-role">{t.role}</div></div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="contact-card reveal reveal-d2">
            <h2 className="contact-card-title">Start a conversation</h2>
            <p className="contact-card-sub">The fastest way to reach us is WhatsApp. Or fill in the form below and we'll reply via WhatsApp or email.</p>
            <a href={`https://wa.me/${WA_NUMBER}?text=Hi%20websitestudio.id!%20I'd%20like%20to%20discuss%20a%20project.`} target="_blank" rel="noopener noreferrer" className="wa-primary-btn">
              <svg viewBox="0 0 24 24" style={{ width:'22px',height:'22px',fill:'white',flexShrink:0 }} aria-hidden="true"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.127.558 4.122 1.532 5.852L0 24l6.334-1.51A11.95 11.95 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22c-1.846 0-3.574-.476-5.083-1.308l-.364-.214-3.77.899.934-3.671-.237-.38A9.947 9.947 0 012 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10z"/></svg>
              Chat on WhatsApp — we reply fast!
            </a>
            <div className="or-divider">or send us a quick brief</div>

            {sent ? (
              <div className="form-success">
                <div className="success-icon">🎉</div>
                <div className="success-text">Message sent to WhatsApp!</div>
                <p className="success-sub">We'll get back to you within 60 minutes. Check your WhatsApp now.</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="enquiry-form" noValidate>
                <div className="form-row">
                  <div className="form-field">
                    ...
                    <input style={inp} type="text" placeholder="Budi Santoso" required value={form.name} onChange={e => setForm(f => ({...f, name: e.target.value}))} onFocus={e => e.target.style.borderColor='var(--purple)'} onBlur={e => e.target.style.borderColor='var(--border2)'} />
                  </div>
                  <div className="form-field">
                    <label>Email</label>
                    <input style={inp} type="email" placeholder="hello@yourcompany.com" value={form.email} onChange={e => setForm(f => ({...f, email: e.target.value}))} onFocus={e => e.target.style.borderColor='var(--purple)'} onBlur={e => e.target.style.borderColor='var(--border2)'} />
                  </div>
                </div>
                <div className="form-field">
                  <label>What do you need?</label>
                  <select style={{...inp, appearance:'none',cursor:'pointer'}} value={form.service} onChange={e => setForm(f => ({...f, service: e.target.value}))} onFocus={e => e.target.style.borderColor='var(--purple)'} onBlur={e => e.target.style.borderColor='var(--border2)'}>
                    <option value="">Select a service…</option>
                    {['Landing Page','Company Website','E-commerce Store','Booking System','Portfolio Site','SaaS / App Landing','Website Redesign','Other'].map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
                </div>
                <div className="form-field">
                  <label>Budget Range</label>
                  <select style={{...inp, appearance:'none',cursor:'pointer'}} value={form.budget} onChange={e => setForm(f => ({...f, budget: e.target.value}))} onFocus={e => e.target.style.borderColor='var(--purple)'} onBlur={e => e.target.style.borderColor='var(--border2)'}>
                    <option value="">Select a range…</option>
                    {['Under Rp 3jt','Rp 3jt – 8jt','Rp 8jt – 20jt','Above Rp 20jt','Not sure'].map(b => <option key={b} value={b}>{b}</option>)}
                  </select>
                </div>
                <div className="form-field">
                  <label>Tell us about your project</label>
                  <textarea style={{...inp, resize:'vertical', minHeight:'90px'}} rows={4} placeholder="Describe your business, what you need, any inspiration sites, timeline…" value={form.message} onChange={e => setForm(f => ({...f, message: e.target.value}))} onFocus={e => e.target.style.borderColor='var(--purple)'} onBlur={e => e.target.style.borderColor='var(--border2)'} />
                </div>
        ...
        <div className="location-card reveal" style={{ marginTop: '4rem' }}>
          ...
          <div className="location-info">
            ...
            <div><strong>websitestudio.id</strong><span>Jakarta, Indonesia · Mon–Sat 09.00–21.00 WIB</span></div>
          </div>
        </div>
      </div>
    </>
