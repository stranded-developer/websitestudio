...
function Modal({ project, onClose }) {
  ...
  return (
    <div
      ...
      style={{ position:'fixed',inset:0,background:'rgba(0,0,0,.88)',backdropFilter:'blur(10px)',zIndex:2000,display:'flex',alignItems:'center',justifyContent:'center',animation:'fadeIn .3s ease' }}
    >
      ...
      <div style={{ background:'var(--surface)',border:'1px solid var(--border)',borderRadius:'22px',padding:'2rem',maxWidth:'920px',width:'90vw',position:'relative',animation:'scaleIn .3s ease' }}>
        <style>{`@keyframes scaleIn{from{transform:scale(.96)}to{transform:scale(1)}}`}</style>
        <button onClick={onClose} style={{ position:'absolute',top:'1rem',right:'1rem',width:'36px',height:'36px',borderRadius:'50%',background:'var(--surface2)',border:'1px solid var(--border2)',color:'var(--text2)',fontSize:'1.1rem',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',transition:'background .2s' }}>✕</button>
        ...
        <div style={{ width:'100%',aspectRatio:'16/9',background:'var(--bg3)',borderRadius:'14px',overflow:'hidden',display:'flex',alignItems:'center',justifyContent:'center' }}>
          <div style={{ textAlign:'center',padding:'3rem',color:'var(--text3)' }}>
            ...
            <p style={{ fontSize:'.875rem',lineHeight:1.6 }}>Add your screen-recording video to this project.<br />Set <code>data-video</code> on the card.</p>
          </div>
        </div>
        <div style={{ marginTop:'1.2rem',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:'1rem' }}>
          <div style={{ display:'flex',gap:'.5rem',flexWrap:'wrap' }}>
            {project.tags.map(t => (
              <span key={t} style={{ background:'var(--purple-glow2)',border:'1px solid var(--border)',borderRadius:'100px',padding:'.28rem .8rem',fontSize:'.72rem',color:'var(--purple-light)' }}>{t}</span>
            ))}
          </div>
          <Link to="/contact" className="btn-primary" style={{ fontSize:'.85rem',padding:'.65rem 1.4rem' }}>Start a Similar Project →</Link>
        </div>
      </div>
    </div>
  )
...
export default function Work() {
  ...
  return (
    <>
      <style>{`
        .work-header { padding-top:calc(var(--nav-h) + 4rem); padding-bottom:3rem; }
      ...
      <div className="page-section work-header">
        ...
        <h1 className="section-title reveal" style={{ fontSize: 'clamp(2.5rem,6vw,5rem)', maxWidth: '700px' }}>
          Sites we've<br /><span style={{ color: 'var(--purple-light)' }}>built & launched</span>
        </h1>
        <p className="section-sub reveal">Every project is designed from scratch — no templates, no shortcuts. Click any card to see the full demo.</p>
            ...
            <article key={p.name} className={`work-card reveal${p.featured ? ' featured' : ''} reveal-d${(i % 3) + 1}`} onClick={() => setModal(p)}>
              <div className={`card-thumb${p.featured ? ' featured-thumb' : ''}`}>
                <div className="thumb-placeholder">
                  ...
                  <span className="thumb-label">Upload your screen recording here</span>
                </div>
        ...
        <div className="reveal" style={{ textAlign: 'center', marginTop: '4rem', paddingTop: '4rem', borderTop: '1px solid var(--border2)' }}>
          <p style={{ color: 'var(--text2)', marginBottom: '1.5rem', fontSize: '1.05rem' }}>Have a project in mind? Let's talk.</p>
          <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link to="/contact" className="btn-primary">Start a Project →</Link>
            <a href={`https://wa.me/${WA_NUMBER}?text=Hi!%20I%20saw%20your%20portfolio%20and%20want%20to%20discuss.`} target="_blank" rel="noopener noreferrer" className="btn-ghost">💬 Chat on WhatsApp</a>
          </div>
        </div>
      </div>
